...
function ModalIndicador({ indicadorSelecionado, carregandoDetalhe, onFechar }) {
  ...
  return createPortal(
    ...
    >
      <div className="flex max-h-[calc(100dvh-1.5rem)] w-full max-w-5xl flex-col overflow-hidden rounded-[1.5rem] border border-slate-200 bg-white shadow-2xl dark:border-slate-800 dark:bg-slate-950 sm:max-h-[calc(100dvh-3rem)]">
        <header className="flex shrink-0 items-start justify-between gap-4 border-b border-slate-200 px-4 py-4 dark:border-slate-800 sm:px-6">
          ...
          <button
            type="button"
            onClick={onFechar}
            className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-700 transition hover:bg-slate-100 focus:outline-none focus:ring-2 focus:ring-slate-400 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100 dark:hover:bg-slate-800"
        ...
        <main className="min-h-0 flex-1 overflow-y-auto p-4 sm:p-6">
          {carregandoDetalhe ? (
            ...
            <div className="space-y-5">
              <section className="rounded-2xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-950">
                ...
                <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                  ...
                  <DetalheResumo
                    label="Módulo"
                    value={indicador.modulo || "—"}
                  />
                  <DetalheResumo
                    label="Janela"
                    value={indicador.janela || "—"}
                  />
                  <DetalheResumo
                    label="Atualizado em"
              ...
              <section className="rounded-2xl border border-blue-200 bg-blue-50 p-4 text-sm leading-6 text-blue-900 dark:border-blue-800 dark:bg-blue-950/40 dark:text-blue-100">
                ...
                <p className="mt-1">
                  Este indicador é derivado da view oficial da Saúde da
                  Plataforma. Para resolver alertas ou críticos, corrija a causa
                  nos módulos de origem, especialmente quando o indicador
                  apontar pendências, certificados, reservas, notificações ou
                  auditoria.
                </p>
              </section>
            </div>
            ...
            <NadaEncontrado
              ...
              mensagem="Não foi possível carregar os dados deste indicador."
            />
          )}
        </main>

        <footer className="flex shrink-0 justify-end border-t border-slate-200 px-4 py-3 dark:border-slate-800 sm:px-6">
          <Botao type="button" variant="contorno" onClick={onFechar}>
            Fechar
          </Botao>
        </footer>
      </div>
