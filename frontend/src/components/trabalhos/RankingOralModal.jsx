// 📁 src/components/trabalhos/RankingOralModal.jsx
// Atualizado em: 15/05/2026
//
// Plataforma Escola da Saúde — v2.0
//
// Modal administrativo de ranking da apresentação oral.
//
// Contratos oficiais usados:
// - PATCH /api/submissao/admin/:id/status
//
// Função:
// - exibir ranking por nota oral;
// - filtrar por chamada, linha temática e busca textual;
// - aprovar submissão para apresentação oral;
// - preservar aprovação de exposição já existente;
// - permitir reprovação administrativa quando necessário.
//
// Diretrizes v2.0:
// - sem status legado aprovado_oral/reprovado;
// - sem confirm nativo;
// - sem toast direto;
// - sem modal antigo;
// - sem listar apenas aprovados oralmente;
// - status oficial: submetida, em_avaliacao, aprovada_exposicao, aprovada_oral, aprovada, reprovada;
// - UX/UI premium real;
// - acessível;
// - mobile-first;
// - dark mode.
...
function lerNotaOral(item) {
  const candidatos = [
    item?.nota_oral,
    item?.media_oral,
    item?.nota_apresentacao_oral,
  ];
...
function patchStatusPayload(action, item) {
  const exposicaoJaAprovada = hasAprovacaoExposicao(item);

  if (action === "aprovar_oral") {
    return {
      payload: {
      ...
      optimistic: {
        status: exposicaoJaAprovada ? "aprovada" : "aprovada_oral",
        status_oral: "aprovado",
        ...(exposicaoJaAprovada ? { status_escrita: "aprovado" } : {}),
        _oral_aprovada: true,
        _exposicao_aprovada: exposicaoJaAprovada,
      },
      message: "Submissão aprovada para apresentação oral.",
    };
  }
...
function Button({
  ...
  const tones = {
    primary:
      "bg-gradient-to-r from-indigo-700 via-emerald-700 to-teal-600 text-white shadow-lg shadow-indigo-900/20 hover:brightness-110",
    slate:
...
function ConfirmBox({ item, action, busy, onCancel, onConfirm }) {
  ...
  return (
    <div className="rounded-3xl border border-indigo-200 bg-indigo-50 p-4 text-sm text-indigo-900 dark:border-indigo-900 dark:bg-indigo-950/30 dark:text-indigo-200">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div className="min-w-0">
          ...
          <p className="mt-2 text-xs opacity-80">
            {approve
              ? "A submissão ficará marcada como aprovada na etapa oral."
              : "A submissão será marcada como reprovada e removida dos rankings ativos."}
          </p>
        </div>
...
export default function RankingOralModal({
  open,
  onClose,
  itens = [],
  onStatusChange,
  ...
  const stats = useMemo(() => {
    ...
    const aprovadasOral = lista.filter(hasAprovacaoOral).length;
      ...
      >
        ...
        >
          <header className="relative overflow-hidden border-b border-white/10 bg-slate-950 p-5 text-white sm:p-6">
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_20%_20%,rgba(79,70,229,.30),transparent_35%),radial-gradient(circle_at_80%_0%,rgba(16,185,129,.25),transparent_35%),radial-gradient(circle_at_70%_80%,rgba(20,184,166,.22),transparent_35%)]" />

            <div className="relative flex items-start justify-between gap-4">
              <div className="min-w-0">
                <div className="mb-3 flex flex-wrap gap-2">
                  <Badge tone="indigo" icon={ShieldCheck}>
                ...
                <div className="mt-4 grid gap-2 lg:grid-cols-[1.4fr_1fr_1fr_auto]">
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="h-12 w-full rounded-2xl border border-white/15 bg-white/10 pl-11 pr-4 text-sm text-white outline-none placeholder:text-white/60 focus:ring-2 focus:ring-indigo-300"
                  ...
                  <div className="relative">
                    ...
                    <select
                      ...
                      className="h-12 w-full rounded-2xl border border-white/15 bg-white/10 pl-11 pr-4 text-sm text-white outline-none focus:ring-2 focus:ring-indigo-300"
                  ...
                  <select
                    ...
                    className="h-12 w-full rounded-2xl border border-white/15 bg-white/10 px-4 text-sm text-white outline-none focus:ring-2 focus:ring-indigo-300"
              ...
              <button
                ...
                ref={closeRef}
                onClick={onClose}
                className="rounded-2xl p-2 text-white/80 transition hover:bg-white/10 hover:text-white focus:outline-none focus:ring-2 focus:ring-indigo-300"
                aria-label="Fechar ranking oral"
              >
          ...
          <div aria-live="polite" className="sr-only">
            {erro || mensagem || `${stats.total} submissões no ranking oral.`}
          </div>
                    ...
                    <table className="w-full min-w-[1180px] text-sm">
                      ...
                      <thead className="bg-slate-950 text-white">
                        <tr>
                          ...
                          <th className="w-28 p-4 text-center">Nota oral</th>
                      ...
                      <tbody>
                        {lista.map((item) => {
                          ...
                          return (
                            <tr
                              key={item.id}
                              className="border-b border-slate-100 transition hover:bg-indigo-50/60 dark:border-slate-800 dark:hover:bg-slate-800/40"
                            >
                              ...
                              <td className="p-4 text-center align-top">
                                <span className="inline-flex items-center justify-center rounded-2xl bg-indigo-600 px-3 py-1.5 text-sm font-black text-white">
                                  {fmtNum(item._nota_oral, 1)}
                                </span>
                              </td>
                              ...
                              <td className="p-4 align-top">
                                <div className="flex flex-wrap justify-center gap-2">
                                  ...
                                  >
                                    {okOral ? "Oral aprovado" : "Aprovar oral"}
                                  </Button>
                ...
                <div className="mt-6 grid gap-3 lg:hidden">
                  {lista.map((item) => {
                    ...
                    return (
                      ...
                      >
                        <div className="h-1.5 bg-gradient-to-r from-indigo-600 via-emerald-600 to-teal-500" />
