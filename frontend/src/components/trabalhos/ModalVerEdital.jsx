...
function Section({ title, icon: Icon, children, tone = "violet" }) {
  ...
  return (
    <section className="rounded-[1.5rem] border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900 sm:p-5">
      <h3
        className={cx("mb-4 flex items-center gap-2 text-lg font-black", color)}
      >
...
export default function ModalVerEdital({ isOpen = true, chamadaId, onClose }) {
  ...
  const chamada = useMemo(() => dados?.chamada || dados || {}, [dados]);
  const linhas = useMemo(
    () => (Array.isArray(dados?.linhas) ? dados.linhas : []),
    [dados?.linhas],
  );
  const criterios = useMemo(
    () => (Array.isArray(dados?.criterios) ? dados.criterios : []),
    [dados?.criterios],
  );
  const criteriosOrais = useMemo(
    () => (Array.isArray(dados?.criterios_orais) ? dados.criterios_orais : []),
  ...
  const limites = useMemo(() => {
    ...
    if (chamada?.limites && typeof chamada.limites === "object") {
      return chamada.limites;
    }
    return {};
  }, [dados?.limites, chamada?.limites]);
  ...
  const carregar = useCallback(async () => {
    ...
    try {
      ...
      setErro(
        getMessage(error, "Não foi possível carregar o edital da chamada."),
      );
    } finally {
      setLoading(false);
    }
  ...
  async function baixarModeloOral() {
    ...
    try {
      ...
      setErro(
        getMessage(
          error,
          "Não foi possível baixar o modelo de apresentação oral.",
        ),
      );
    } finally {
    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          className="flex w-full max-w-6xl flex-col overflow-hidden rounded-[1.75rem] border border-white/20 bg-white shadow-2xl outline-none dark:bg-slate-950 sm:rounded-[2rem]"
          style={{
        ...
        >
          <header className="relative shrink-0 overflow-hidden border-b border-white/10 bg-slate-950 px-5 py-4 text-white sm:px-6 sm:py-5">
            {" "}
            ...
            <div className="relative flex items-start justify-between gap-4">
              ...
              <button
                ...
                ref={closeButtonRef}
                onClick={onClose}
                className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl text-white/85 transition hover:bg-white/10 hover:text-white focus:outline-none focus:ring-2 focus:ring-violet-300"
            ...
            <>
                  ...
                  <aside className="space-y-4">
                    ...
                    >
                      <dl className="space-y-3 text-sm">
                        ...
                        />
                        <InfoRow
                          label="Máximo de coautores"
                          value={minis.coautores}
                        />
                      </dl>
                    </Section>
                    ...
                    >
                      <div className="space-y-2">
                        ...
                        <p className="text-xs leading-relaxed text-slate-500 dark:text-slate-400">
                          Caso algum modelo ainda não tenha sido importado pela
                          administração, o sistema exibirá uma mensagem de
                          indisponibilidade.
                        </p>
                      </div>
                    </Section>
                    ...
                    <Section title="Orientação" icon={Sparkles} tone="cyan">
                      <p className="text-sm leading-relaxed text-slate-600 dark:text-slate-300">
                        Leia as normas antes de submeter. Após o trabalho entrar
                        em avaliação, alterações pelo autor podem ficar
                        bloqueadas para preservar a rastreabilidade.
                      </p>
                    </Section>
                  </aside>
                </div>
              ...
              <footer className="shrink-0 border-t border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-950">
                <div className="flex flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <p className="text-xs leading-relaxed text-slate-500 dark:text-slate-400">
                    Confira o edital completo antes de submeter ou editar seu
                    trabalho.
                  </p>
        ...
        >
          <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
            ...
            <div className="flex flex-wrap gap-1.5">
              <Badge tone="slate">
                Escala {criterio?.escala_min ?? "—"}–
                {criterio?.escala_max ?? "—"}
              </Badge>
              <Badge tone="slate">Peso {criterio?.peso ?? 1}</Badge>
            </div>
          </div>
        </div>
      ))}
...
ModalVerEdital.propTypes = {
  ...
  chamadaId: PropTypes.oneOfType([PropTypes.number, PropTypes.string])
    .isRequired,
